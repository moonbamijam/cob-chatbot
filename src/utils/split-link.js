...
export const splitLinkToResponse = (string, intent) => {
  ...
  if (link && intent?.startsWith("form")) {
    const interpolatedString = string.replace(
      matchLinkPattern,
      `<a href="${link[1]}" class="text-blue-500 font-semibold uppercase hover:underline" target="_blank">click here to download</a>`
    );
    return interpolatedString;
  } else if (link && intent?.startsWith("link")) {
    const interpolatedString = string.replace(
      matchLinkPattern,
      `<a href="${link[1]}" class="text-blue-500 font-semibold uppercase hover:underline" target="_blank">click here to view</a>`
    );
    return interpolatedString;
  } else return string;
};
